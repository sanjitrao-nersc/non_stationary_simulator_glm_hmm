...
def load_dale_spikes(file_path='test_dale.spikes.npz'):
    """
...
def plot_inferred_dynamics_with_lambda(model, spikes, inputs, title="Inferred States & Poisson Intensity"):
    """
    ...
    # 2. Calculate the 'Active' Poisson Lambda (Mean Firing Rate) [cite: 3423, 3464]
    expected_rates = np.zeros(T)
    for t in range(T):
        curr_state = predicted_states[t]
        
        # model.observations.params[curr_state] is shape (N_neurons, input_dim)
        # inputs[t] is shape (input_dim,)
        # We calculate the log_lambda for each neuron, then take the mean exp
        log_lambdas = model.observations.params[curr_state] @ inputs[t] 
        expected_rates[t] = np.mean(np.exp(log_lambdas)) # Hz [cite: 3571]
    ...
    # Overlaid Poisson Intensity (The 'Hidden' Mean) [cite: 3464, 3526]
    ax_lambda = axes[1].twinx()
    ax_lambda.fill_between(time, expected_rates, color='red', alpha=0.15, label="Expected Rate (λ)")
    ax_lambda.plot(time, expected_rates, color='red', linewidth=1, alpha=0.6, linestyle='--')
    ax_lambda.set_ylabel("Predicted Firing Rate (Hz)", color='red')
    ...
    plt.savefig("inferred_ns_state_dynamics_with_lambda.png")
    print("\nInferred state dynamics plot WITH LAMBDA saved as inferred_ns_state_dynamics_with_lambda.png. ")
...
def main():
    ...
    ## PLOTTING
    #plot_inferred_dynamics_with_lambda(trained_model, y_sess, x_sess)    
